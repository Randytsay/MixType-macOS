...
/// LibVanguard 對宿主（MainAssembly4Darwin）的動作依賴注入點。
///
/// 本套件不得依賴 MainAssembly4Darwin；所有需要宿主服務的動作
/// （LXMgr、IMEApp、Notifier、AppDelegate、SessionUI 等）皆由宿主於啟動時
...
public final class SessionHost {
  ...
  /// 儲存 MixType 組句短語學習 pending observations。
  public var saveCompositionPhraseLearningData: (_ isCHS: Bool) -> () = { _ in }
  /// 儲存 MixType 無聲調拼音單字偏好。
  public var saveSingleCharacterPreferenceData: (_ isCHS: Bool) -> () = { _ in }
  /// 直接寫入使用者語彙。
  ...
  public var checkIfPhrasePairIsFiltered: (_ userPhrase: String, _ mode: Shared.InputMode, _ keyArray: [String])
    ...
    { _, _, _ in false }
  /// 取得使用者辭典資料路徑。
  public var userDictDataURL: (_ mode: Shared.InputMode, _ type: LXAssembly.ReplacableUserDataType) -> URL =
...
// MARK: - Shared.InputMode.lexicon（跨平台）

extension Shared.InputMode {
  private struct LexiconCache {
    // MARK: Lifecycle

    init() {
      self.chs = LXAssembly.LXFacade(
        isCHS: true,
        pomDataURL: SessionHost.shared.pomDataURL(.imeModeCHS)
      )
      self.cht = LXAssembly.LXFacade(
        isCHS: false,
    ...
    let cht: LXAssembly.LXFacade
    let chs: LXAssembly.LXFacade

    func model(for mode: Shared.InputMode) -> LXAssembly.LXFacade {
      switch mode {
  ...
  private static var productionCache = LexiconCache()
  private static var unitTestCache: LexiconCache?

  private static var activeCache: LexiconCache {
    if UserDefaults.pendingUnitTests {
      if unitTestCache == nil {
        unitTestCache = LexiconCache()
      }
  ...
  public static func resetLexiconCache(forUnitTests: Bool? = nil) {
    switch forUnitTests {
    case true?:
      unitTestCache = nil
    case false?:
      productionCache = LexiconCache()
    case nil:
      productionCache = LexiconCache()
      unitTestCache = nil
    }
  ...
  public var lexicon: LXAssembly.LXFacade {
    switch self {
    case .imeModeNULL:
      return .init()
    default:
